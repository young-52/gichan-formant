...
from utils.math_utils import (
    hz_to_bark, bark_to_hz, remove_outliers_mahalanobis,
    lobanov_normalization, gerstman_normalization,
    watt_fabricius_normalization, bigham_normalization,
...
class MainController:
    ...
    def _build_outlier_log_message(self, total_removed, file_removed, files_with_small_labels, any_label_tested):
        ...
        if files_with_small_labels:
            ...
            detail = " / ".join(parts)
            return config.LOG_MSG["OUTLIER_NOT_REMOVED_MIN_LABELS"].format(detail=detail)
        if any_label_tested:
    ...
    def on_outlier_mode_changed(self):
        ...
        for item in self.plot_data_list:
            ...
            filtered_df, n_removed, _, meta = remove_outliers_mahalanobis(df_orig, plot_type, outlier_mode)
            item['df'] = filtered_df
        ...
        msg = self._build_outlier_log_message(total_removed, file_removed, files_with_small_labels, any_label_tested)
        if msg:
            app_logger.info(msg)
    ...
    def add_files(self, filepaths):
        ...
        result["total_files"] = len(self.filepaths)
        result["has_f3_all"] = all(d['has_f3'] for d in self.plot_data_list) if self.plot_data_list else False
        return result
        ...
        if result["failed"]:
            ...
            for name, errs in result["failed"][:3]:
                if errs:
                    ...
                    app_logger.debug(config.LOG_MSG["FILE_LOAD_FAILED_DEBUG"].format(name=name, msg=msg))
    ...
    def remove_file(self, index):
        ...
        if index < 0 or index >= len(self.plot_data_list):
            ...
            app_logger.debug(config.LOG_MSG.get("FILE_REMOVE_INDEX_INVALID", "[DEBUG] remove_file: 잘못된 인덱스 요청"))
            return
        ...
        app_logger.info(config.LOG_MSG["FILE_REMOVED"].format(removed_name=removed_name))
    ...
    def get_initial_open_dir(self):
        ...
        return QStandardPaths.writableLocation(QStandardPaths.StandardLocation.DocumentsLocation) or ""
    ...
    def _render_live_preview_content(self, current_data, params, smart_ranges, default_design):
        """LIVE 모니터에 플롯을 그려 버퍼로 저장한 뒤 레이블에 표시하고 하단 정보를 갱신합니다."""
        self.live_preview_fig.clear()
        *_, _ = self.plot_engine.draw_plot(self.live_preview_fig, current_data['df'], params,
                                           manual_ranges=smart_ranges, design_settings=default_design)
        ...
        if hasattr(self.ui, 'preview_info_label'):
            ...
            x_names = {'f1_f2': 'F2', 'f1_f3': 'F3', 'f1_f2_prime': "F2'", 'f1_f2_minus_f1': 'F2-F1', 'f1_f2_prime_minus_f1': "F2'-F1"}
    ...
    def _flush_live_preview(self):
        ...
        smart_ranges = self._get_smart_ranges(params['type'], params['use_bark_units'], params['f1_scale'],
                                              params['f2_scale'])
        default_design = self._get_default_design()
        try:
            self._render_live_preview_content(current_data, params, smart_ranges, default_design)
        except Exception as e:
            traceback.print_exc()
            try:
                self.live_preview_fig.clear()
                ax = self.live_preview_fig.add_subplot(111)
                ax.text(0.5, 0.5, "LIVE 렌더링 오류", ha='center', va='center',
                        fontfamily=kor_font, fontsize=11)
                ax.set_axis_off()
            except Exception:
                pass
            self.ui.preview_label.clear()
    ...
    def generate_plot(self):
        ...
        if not self.plot_data_list:
            QMessageBox.warning(self.ui, "데이터 없음", "분석할 데이터를 먼저 로드해 주세요.")
            return

        fig = Figure(figsize=(6.5, 6.5), dpi=100)
        plot_type = self.ui.get_plot_type()
        x_axis_labels = {
            'f1_f2': 'F2', 'f1_f3': 'F3', 'f1_f2_prime': "F2'",
            'f1_f2_minus_f1': 'F2 - F1', 'f1_f2_prime_minus_f1': "F2' - F1"
        }
        x_label = x_axis_labels.get(plot_type, 'X-Axis')

        popup = PlotPopup(parent=self.ui, controller=self, figure=fig, x_axis_label=x_label)
        ...
        custom_offsets = self.custom_label_offsets.get((popup.current_idx, plot_type_fixed), {})
        layer_overrides = popup.get_layer_design_overrides()
        _, snapping_data, label_data, label_text_artists = self.plot_engine.draw_plot(
            fig, current_data['df'], popup.fixed_plot_params,
            manual_ranges=smart_ranges, filter_state=filter_state,
            design_settings=ds_settings, custom_label_offsets=custom_offsets,
            layer_overrides=layer_overrides)
        popup.set_draw_result(snapping_data, label_data, label_text_artists, (popup.current_idx, plot_type_fixed))
        popup.canvas.draw()
    ...
    def open_compare_dialog(self, current_idx, parent_window=None):
        ...
        if len(self.plot_data_list) < 2:
            QMessageBox.warning(parent_window or self.ui, "데이터 부족", "비교할 대상이 부족합니다.\n2개 이상의 데이터를 로드해 주세요.")
            return
    ...
    def open_compare_plot(self, current_idx, target_idx, normalization=None, parent_window=None):
        ...
        try:
            ...
            popup = ComparePlotPopup(parent_window or self.ui, self, fig, current_idx, target_idx, x_axis_label=x_label, normalization=normalization)
            ...
            popup.show()
            QTimer.singleShot(0, lambda: self.refresh_compare_plot(fig, popup.canvas, popup.range_widgets, None, popup,
    ...
    def refresh_compare_plot(self, figure, canvas, range_widgets, lbl_info, popup_window, idx_blue, idx_red):
        """다중 비교 플롯 창의 범위를 적용하고 캔버스를 갱신합니다. (0xC0000409 방지: 예외 처리 및 유효성 검사)"""
        if figure is None or canvas is None or range_widgets is None or popup_window is None:
            return
        try:
            manual_ranges = {
            ...
            if hasattr(popup_window, 'cb_sigma') and popup_window.cb_sigma is not None:
                try:
                    popup_window.fixed_plot_params = dict(
                        popup_window.fixed_plot_params or {}, sigma=float(popup_window.cb_sigma.currentText()))
            ...
            if norm and hasattr(self.plot_engine, 'draw_compare_normalized'):
                ...
                popup_window.fixed_plot_params = dict(popup_window.fixed_plot_params or {}, normalization=norm)
                ...
                ds_settings = popup_window.get_design_settings() or self._get_default_design()
                sigma = popup_window.fixed_plot_params.get('sigma', 2.0) if popup_window.fixed_plot_params else 2.0
                ...
                _, snapping_data, label_data_blue, label_data_red, label_text_artists_blue, label_text_artists_red = self.plot_engine.draw_compare_normalized(
                    figure, df_blue_norm, df_red_norm, norm, name_blue=name_blue, name_red=name_red,
                    filter_state_blue=fs_blue, filter_state_red=fs_red, design_settings=ds_settings, sigma=sigma,
                    custom_label_offsets_blue=custom_blue, custom_label_offsets_red=custom_red,
                ...
                _, snapping_data, label_data_blue, label_data_red, label_text_artists_blue, label_text_artists_red = self.plot_engine.draw_multi_plot(
                ...
                ax.text(0.5, 0.5, "[알림] plot_engine.py 내에\ndraw_multi_plot() 구현이 필요합니다.",
                        ha='center', va='center', fontfamily=kor_font, fontsize=12)
            ...
            if self.ruler_tool.active and figure.axes:
                ...
                if not r_design and getattr(popup_window, 'design_tab', None):
                    r_design = getattr(popup_window.design_tab, 'get_current_settings', lambda: {})()
                self.ruler_tool.set_context(canvas, figure.axes[0], popup_window.fixed_plot_params,
                                            popup_window.snapping_data, r_design or None)
            if self.label_move_tool and self.label_move_tool.active and figure.axes:
                ...
                if series:
                    ...
                    design = getattr(popup_window, 'design_settings', None) or (
                                getattr(popup_window, 'design_tab', None) and getattr(popup_window.design_tab,
                                                                                      'get_current_settings',
                                                                                      lambda: {})())
                    ell_color = (design.get(series) or {}).get('ell_color',
                                                               '#1976D2' if series == 'blue' else '#E64A19')
                    # 여기서 텍스트 아티스트(lta)도 함께 전달
                    self.label_move_tool.set_context(canvas, figure.axes[0], ld, highlight_color=ell_color,
            ...
            try:
                figure.clear()
                ax = figure.add_subplot(111)
                ax.text(0.5, 0.5, "다중 플롯 렌더링 오류", ha='center', va='center',
                        fontfamily=kor_font, fontsize=11)
                ax.set_axis_off()
                canvas.draw()
            except Exception:
    ...
    def refresh_plot(self, figure, canvas, range_widgets, lbl_info, popup_window):
        ...
        try:
            ...
            _, snapping_data, label_data, label_text_artists = self.plot_engine.draw_plot(
                figure, current_data['df'], popup_window.fixed_plot_params,
                manual_ranges=manual_ranges, filter_state=filter_state,
                design_settings=ds_settings, custom_label_offsets=custom_offsets,
                layer_overrides=layer_overrides)
            popup_window.set_draw_result(snapping_data, label_data, label_text_artists, (idx, plot_type))
            canvas.draw()
            ...
            if self.ruler_tool.active:
                ...
                self.ruler_tool.set_context(canvas, figure.axes[0], popup_window.fixed_plot_params, snapping_data,
                                            r_design or None)
            if self.label_move_tool and self.label_move_tool.active:
                self.label_move_tool.set_context(canvas, figure.axes[0], label_data, label_text_artists=label_text_artists)
        except Exception as e:
            traceback.print_exc()
            ...
            try:
                figure.clear()
                ax = figure.add_subplot(111)
                ax.text(0.5, 0.5, "플롯 렌더링 오류", ha='center', va='center',
                        fontfamily=kor_font, fontsize=11)
                ax.set_axis_off()
                canvas.draw()
            except Exception:
    ...
    def navigate_plot(self, direction, figure, canvas, lbl_info, popup_window, range_widgets):
    ...
    def toggle_ruler(self, popup_window):
        ...
        if not self.ruler_tool.active:
            ...
            if popup_window.figure.axes:
                ...
                design = popup_window.get_design_settings() if hasattr(popup_window, 'get_design_settings') else {}
                if not design and getattr(popup_window, 'design_tab', None):
                    design = getattr(popup_window.design_tab, 'get_current_settings', lambda: {})()
                self.ruler_tool.set_context(popup_window.canvas, popup_window.figure.axes[0],
                                            popup_window.fixed_plot_params, snapping_data, design or None)
    ...
    def _save_label_offset(self, dragging, popup_window):
        ...
        self.refresh_plot(popup_window.figure, popup_window.canvas, popup_window.range_widgets,
                          popup_window.lbl_info, popup_window)
    ...
    def toggle_label_move(self, popup_window):
        ...
        self.label_move_tool.on_offset_saved = (lambda pw: lambda d: self._save_label_offset(d, pw))(popup_window)
        self.label_move_tool.on_offset_cleared = (lambda pw: lambda v: self._clear_label_offset(pw, v))(popup_window)
        ...
        if not self.label_move_tool.active and popup_window.figure.axes:
            self.label_move_tool.set_context(
                popup_window.canvas, popup_window.figure.axes[0],
    ...
    def _save_compare_label_offset(self, dragging, popup_window, series):
        ...
        dy = dragging['ly'] - dragging['cy']
        self.custom_label_offsets.setdefault(key, {})[dragging['vowel']] = (dx, dy)
        self.refresh_compare_plot(popup_window.figure, popup_window.canvas, popup_window.range_widgets,
                                  None, popup_window, popup_window.idx_blue, popup_window.idx_red)
    ...
    def toggle_compare_label_move(self, popup_window, series):
        ...
        if self.label_move_tool.active and old_series and old_series != series:
            # 타겟 저장/원상복귀 함수 교체
            self.label_move_tool.on_offset_saved = (lambda pw, s: lambda d: self._save_compare_label_offset(d, pw, s))(
                popup_window, series)
            self.label_move_tool.on_offset_cleared = (lambda pw, s: lambda v: self._clear_compare_label_offset(pw, s, v))(
                popup_window, series)

            # 넘어갈 탭(새로운 series)의 데이터와 텍스트 아티스트를 가져옴
            label_data = getattr(popup_window, 'label_data_' + series, [])
            label_text_artists = getattr(popup_window, 'label_text_artists_' + series, [])
            design = getattr(popup_window, 'design_settings', None) or (
                getattr(popup_window.design_tab, 'get_current_settings', lambda: {})())
            ell_color = (design.get(series) or {}).get('ell_color', '#1976D2' if series == 'blue' else '#E64A19')

            # 툴을 끄지 않고(detach 안 함), 포인터가 바라보는 타겟만 즉시 교체!
            self.label_move_tool.set_context(popup_window.canvas, popup_window.figure.axes[0], label_data,
                                             highlight_color=ell_color, label_text_artists=label_text_artists)
            ...
            app_logger.info(config.LOG_MSG["LABEL_MOVE_SERIES"].format(
        ...
        if on_now:
            app_logger.info(config.LOG_MSG["LABEL_MOVE_ON_SERIES"].format(
                series='기준' if series == 'blue' else '비교'
            ))
        else:
            app_logger.info(config.LOG_MSG["LABEL_MOVE_OFF"])
    ...
    def download_plot(self, figure, fmt, parent_window=None):
        ...
        if parent_window and getattr(parent_window, 'idx_blue', None) is not None and getattr(parent_window, 'idx_red', None) is not None:
            name_blue = os.path.splitext(self.plot_data_list[parent_window.idx_blue]['name'])[0]
            name_red = os.path.splitext(self.plot_data_list[parent_window.idx_red]['name'])[0]
            base = f"{name_blue}_{name_red}{outlier_suffix}"
            norm = getattr(parent_window, 'normalization', None)
            if norm:
                base += "_" + {'Lobanov': 'Lobanov', 'Gerstman': 'Gerstman', '2mW/F': '2mWF', 'Bigham': 'Bigham'}.get(norm, norm.replace('/', '').replace(' ', ''))
        ...
        if file_path:
            try:
                ...
                traceback.print_exc()
                QMessageBox.critical(parent_window or self.ui, "저장 실패", f"저장 중 오류가 발생했습니다:\n{e}")

    def batch_download_with_options(self, ranges, sigma, img_format, design_settings=None):
        ...
        progress_dialog = QProgressDialog("이미지 저장 중...", "취소", 0, total, self.ui)
        ...
        worker = BatchSaveWorker(
            save_dir, self.plot_data_list, self.plot_engine, plot_params,
            ranges, ds_settings, img_format
        )
        ...
        def on_finished(success_count):
            ...
            if success_count == 0 and errors:
                ...
                app_logger.warning(config.LOG_MSG["BATCH_ALL_FAILED"].format(
                    fail_count=len(errors), sample=sample
                ))
                QMessageBox.warning(self.ui, "일괄 저장 실패", config.LOG_MSG["BATCH_ALL_FAILED_BOX"])
            else:
                app_logger.info(config.LOG_MSG["BATCH_SUCCESS"].format(success_count=success_count))
                QMessageBox.information(self.ui, "일괄 저장 완료", f"총 {success_count}개의 이미지가 '{save_dir}'에 저장되었습니다.")
    ...
    def get_outlier_mode(self):
        ...
        return self.ui.get_outlier_mode() if hasattr(self.ui, 'get_outlier_mode') else None
    ...
    def get_smart_ranges_for_params(self, plot_type, use_bark=False, f1_scale=None, f2_scale=None):
        """플롯 타입·스케일에 따른 축 범위 dict. View/팝업은 이 공개 메서드만 호출."""
        return self._get_smart_ranges(plot_type, use_bark, f1_scale, f2_scale)

    # --- 유틸리티 메서드 ---

    def _get_smart_ranges(self, plot_type, use_bark=False, f1_scale=None, f2_scale=None):
        ...
        return {
            'y_min': str(y_rc['y_min']), 'y_max': str(y_rc['y_max']),
